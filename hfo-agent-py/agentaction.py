...
#        actionDist = actionJSON[subfieldIndex]["actionDist"]
    ...
    actions = [ (Action(actionenum = MOVE),      actionDist[0][1])
              ...
              , (Action(actionenum = SHOOT),     actionDist[2][1])
              , (Action(actionenum = DRIBBLE),   actionDist[3][1])
              , (Action(actionenum = NOOP),      actionDist[4][1])
  #            , (Action(actionenum = MOVE_TO, arguments = actionJSON[4][0]["arguments"]), actionJSON[subfieldIndex]["actionDist"] [4][1])
              ]

    # print("{0}").format(actionDist[0][1])
    # print("{0}").format(actionDist[1][1])
    # print("{0}").format(actionDist[2][1])
    # print("{0}").format(actionDist[3][1])
    # print("{0}").format(actionDist[4][1])
...
class Action(object):
    ...
    def execute(self, env, state = []):
        ...
        if self.actionenum == MOVE:      print("Doing MOVE"); env.act(MOVE); 
        if self.actionenum == INTERCEPT: print("Doing INTERCEPT"); env.act(INTERCEPT);
        if self.actionenum == CATCH:     print("Doing CATCH"); env.act(CATCH);
        if self.actionenum == NOOP:      print("Doing NOOP"); env.act(NOOP);
        if self.actionenum == SHOOT:     print("Doing SHOOT"); env.act(SHOOT);
        if self.actionenum == DRIBBLE:   print("Doing DRIBBLE"); env.act(DRIBBLE);
        if self.actionenum == PASS:      print("Doing PASS"); env.act(PASS, self.arguments[0]);
